...
import 'package:fap_properties/views/tenant/tenant_New_contract/make_payment/make_payment_new.dart';
import 'package:fap_properties/views/widgets/snackbar_widget.dart';
import 'package:get/get.dart';
import 'package:intl/intl.dart';

class OnlinePaymentsNewContractController extends GetxController {
  ...
  Future<int> getOnlinePayable() async {
    try {
      ...
      // var resp = await TenantRepository.getContractOnlinePayablNew(
      //     SessionController().getContractID());
      print(resp);
  ...
  void registerPayment(String contractNo) async {
    ...
    // var resp = await TenantRepository.registerPaymentNew(
    //     TenantRegisterPaymentModel(
    //         totalAmount: isPayemntValue.value == 1
    //             ? double.parse(sumOfSelectedPayments.value.replaceAll(',', ''))
    //             : double.parse(
    ...
    if (resp is RegisterPaymentResponse) {
      ...
      if (resp.url == null || resp.transactionId == null) {
        ...
        Get.to(
            () => MakePaymentNewContract(data: resp, contractNo: contractNo));
      }
    } else {
