...
class VerifyUserOtpController extends GetxController {
  ...
  void resendOtpBtn() async {
    ...
    try {
      ...
      var response = await CommonRepository.validateUser();
      resending.value = false;
      if (response is ValidateUserModel) {
        validateUserModel.value = response;
        SessionController().setOtpCode(response.otpCode);
        resendCounter.value++;
...
//           if (response.data.length == 1) if (response.data[0].roleId == 4) {
//             validatePublicRole();
//             return;
//           }
//           if (model.value.user.mpinSet && !SessionController().getResetMpin())
